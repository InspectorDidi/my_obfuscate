      ...
      input_io.each_line(chomp: false) do |line|
        if table_data = parse_insert_statement(line)
          ...
          if config[table_name]?
            ...
            MyObfuscate.log.warn { "Deprecated: #{table_name} was not specified in the config.  A future release will cause this to be an error.  Please specify the table definition or set it to :keep." }
