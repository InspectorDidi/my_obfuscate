  ...
  describe ".apply_table_config" do
    ...
    describe "conditional directives" do
      ...
      it "should supply the original row values to the conditional" do
        new_row = MyObfuscate::ConfigApplicator.apply_table_config(["blah", "something_else"], {:a => {:type => :fixed, :string => "123"}, :b => {:type => :fixed, :string => "yup", :if => ->(row : Hash(Symbol, String | Nil)) { row[:a] == "blah" }}}, [:a, :b])
        expect(new_row[0]).to eq("123")
      ...
      it "should honor combined :unless and :if conditionals" do
        #both true
        new_row = MyObfuscate::ConfigApplicator.apply_table_config(["blah", "something_else", "5"], {:a=> {:type => :fixed, :string => "123", :if => ->(row : Hash(Symbol, String | Nil)) { row[:a] == "blah" }, :unless => ->(row : Hash(Symbol, String | Nil)) { row[:b] == "something_else" }}}, [:a, :b, :c])
        expect(new_row[0]).to eq("blah")

        #both false
        new_row = MyObfuscate::ConfigApplicator.apply_table_config(["blah", "something_else", "5"], {:a=> {:type => :fixed, :string => "123", :if => ->(row : Hash(Symbol, String | Nil)) { row[:a] == "not blah" }, :unless => ->(row : Hash(Symbol, String | Nil)) { row[:b] == "not something_else" }}}, [:a, :b, :c])
        expect(new_row[0]).to eq("blah")

        #if true, #unless false
        new_row = MyObfuscate::ConfigApplicator.apply_table_config(["blah", "something_else", "5"], {:a=> {:type => :fixed, :string => "123", :if => ->(row : Hash(Symbol, String | Nil)) { row[:a] == "blah" }, :unless => ->(row : Hash(Symbol, String | Nil)) { row[:b] == "not something_else" }}}, [:a, :b, :c])
        expect(new_row[0]).to eq("123")
